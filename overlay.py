...
class Overlay:
    ...
    def show_stats(self, summary: str) -> None:
        """Display session statistics in the overlay (F7)."""
        self._last_text = summary
        self._last_bg_colour = "#1a1a4e"
        def apply_stats() -> None:
            self.frame.configure(bg="#1a1a4e")
            self.body.configure(text=summary, bg="#1a1a4e", justify=tk.LEFT)
        self.root.after(0, apply_stats)
