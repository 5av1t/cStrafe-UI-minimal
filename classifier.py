...
class AxisState:
    ...
    def classify_shot(self, shot_time: float) -> Tuple[str, Optional[float], Optional[float], Optional[str]]:
        # Capture direction before reset clears the keys
        direction = None
        if self.cs_release_key and self.cs_press_key:
            direction = f"{self.cs_release_key}→{self.cs_press_key}"
        elif self.cs_release_key:
            direction = f"{self.cs_release_key}→?"
        ...
        if self.overlap_start_time is not None:
            if not (
                ...
                self._reset()
                return "Overlap", overlap_time, None, direction
        if (
            self.cs_press_time is not None
            and self.cs_release_time is not None
            and self.cs_press_time > self.cs_release_time
            ...
            self._reset()
            return "Counter‑strafe", cs_time, shot_delay, direction
        self._reset()
        return "Bad", None, None, direction
...
class MovementClassifier:
    ...
    def classify_shot(self, shot_time: float) -> ShotClassification:
        v_label, v_val1, v_val2, v_dir = self.vertical.classify_shot(shot_time)
        h_label, h_val1, h_val2, h_dir = self.horizontal.classify_shot(shot_time)
        negativity = {
            "Overlap": 2,
            "Counter‑strafe": 1,
            "Bad": 0,
        ...
        if v_score > h_score:
            label, val1, val2, direction = v_label, v_val1, v_val2, v_dir
        elif h_score > v_score:
            label, val1, val2, direction = h_label, h_val1, h_val2, h_dir
        else:
        ...
        if label == "Counter‑strafe":
            return ShotClassification(label=label, cs_time=val1, shot_delay=val2, direction=direction)
        elif label == "Overlap":
            return ShotClassification(label=label, overlap_time=val1, direction=direction)
        return ShotClassification(label=label, direction=direction)