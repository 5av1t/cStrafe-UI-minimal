...
class InputListener:
    ...
    def _on_key_press(self, key: keyboard.Key) -> None:
        ...
        if key == keyboard.Key.f7:
            self.overlay.show_stats(self._logger.get_summary())
            return
    ...
    def _build_classification(self, base: ShotClassification, shot_time: float) -> ShotClassification:
        ...
        if base.label == "Overlap":
            return ShotClassification(label="Overlap", overlap_time=base.overlap_time, direction=direction)
        if base.label == "Counter‑strafe":
            ...
            if cs_time is not None and shot_delay is not None:
                if shot_delay > 230.0 or (cs_time > 215.0 and shot_delay > 215.0):
                    return ShotClassification(label="Bad", cs_time=cs_time, shot_delay=shot_delay, direction=direction)
                return ShotClassification(label="Counter‑strafe", cs_time=cs_time, shot_delay=shot_delay, direction=direction)
            return ShotClassification(label="Bad", direction=direction)
        return ShotClassification(label="Bad", direction=direction)